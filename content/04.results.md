...
Following our criteria for human assessments (see above), we inspected the PRs generated by the artificial intelligence-based workflow and reported on our assessment of the changes suggested by the tool across different sections of the manuscripts.
...
We also observed a high-quality revision of the introduction of PhenoPLIER using the manubot platform.
However, the artificial intelligence model failed to maintain the format of citations in one paragraph.
Additionally, the model did not converge to a revised text for the last paragraph, and our software tool left an error message as an HTML comment at the top: `The AI model returned an empty string`.
...
As human authors, we would make a single change at the end of this sentence to avoid repeating the word "complexity": "..., while CCC increased the model's complexity to capture the relationships." The revised paragraph is more concise and clearly describes what the figure shows and how CCC works.
...
For instance, the Manubot-generated revised text for certain paragraphs was more concise, direct, and clear.
...
This process highlights the potential benefits and challenges of using large language models in scholarly publishing software.
